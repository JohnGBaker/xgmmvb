...
\begin{align}
  ...
  %q_\theta(\bm\theta|\bm\eta)&=\mathcal{D}\left(\bm\alpha|\bm\lambda\right)\prod_{j=1}^\kappa\mathcal{NW}\left(\mu_j^a,\Gamma_{j\,ab}|\rho_j^a,\beta_j,V_{j\,ab},\nu_j\right)\label{eq:q-th-GMM}\\
  %
%  \log q_\theta(\bm\theta|\bm\eta)&=\log\mathcal{D}\left(\bm\alpha|\bm\lambda\right)+\sum_{j=1}^\kappa\log\mathcal{NW}\left(\mu_j^a,\Gamma_{j\,ab}|\rho_j^a,\beta_j,V_{j\,ab},\nu_j\right)\label{eq:q-th-GMM}\\
  %
  ...
  %
  %\E{\mathcal{D}(\bm\lambda)}\left[\ln{\mathcal{D}[\bm\lambda']}\right]
  %&=(\kappa-\bar\lambda')\psi(\bar\lambda)+\ln\Gamma(\bar\lambda')\nn\\
  %&\qquad+\sum_{i=1}^\kappa\left[(\lambda'_i-1)\psi(\lambda_i)-\ln\Gamma(\lambda'_i)\right]\label{eq:ElnD}\\
  %
  F(\bm\eta,\bm\gamma^*|\kappa)
  ...
  % used:
      %\E{\mathcal {NW}[\bm\eta]}\left[\ln\mathcal{NW}[\bm\eta']\right]\Big]
      ...
      %&\qquad- \frac D2\left(\psi_D\left(\frac\nu2\right)+\ln\left(2^{D+1}\pi\left|V\right|\right)\right)\label{eq:ENW_expanded}
\end{align}
...
\begin{align}
  ...
  %&\approx\sum_{i=1}^N\log\bar\gamma^*_i\nn\\
  %&\quad+\frac12\ln N + N\nn\\
  %&\quad+\sum_{j=1}^\kappa\left[-\frac12\ln N_j-N_j\right]\nn\\
  %&\quad+\sum_{j=1}^\kappa\Big[ \frac{N_j}2\ln|\Sigma_j|-\frac {D(3+D)}4\ln N_j\nn\\
    %&\qquad-\frac12N_j\left(\ln\left|2\Sigma_j\right|-D\ln N_j\right)\nn\\
    %&\qquad-\frac12N_jD\ln\frac{N_j}2\Big]\nn\\
  &\approx\sum_{i=1}^N\log\bar\gamma^*_i+\frac12\ln N-\sum_{j=1}^\kappa\frac {D(3+D)+2}4\ln N_j
  %A(\bm\eta)&\approx\frac N2\left(\ln|\Sigma|\right)-\frac {D(3+D)}4\ln N\nn\\
  %\psi_D(N/2)&\approx D\ln\left(\frac{N}2\right)-\frac{D(D+1)}{2N}\nn\\
\end{align}
...
\begin{align}
  ...
  %&\approx\frac{D^2}{2}\ln\pi+\frac D2\ln 2+\sum_{j=1}^D{\left(\frac{N-j}2\ln(\frac{N+1-j}2)-\frac{N+1-j}2\right)}\nn\\
  %&\approx\frac{D^2}{2}\ln\pi+\frac D2\ln 2+\sum_{j=1}^D{\left(\frac{N-j}2(\ln(\frac N2)+\ln(1+\frac{1-j}N)-\frac{N+1-j}2\right)}\nn\\
  %&\approx\frac{D^2}{2}\ln\pi+\frac D2\ln 2+\sum_{j=1}^D{\left(\frac{N-j}2\ln\frac N2+\frac{N}2+\frac{1-2j}2-\frac{N+1-j}2\right)}\nn\\
  %&\approx\frac{D^2}{2}\ln\pi+\frac D2\ln 2+\sum_{j=1}^D{\left(\frac{N-j}2\ln\frac N2-\frac{j}2\right)}\nn\\
  %&\approx\frac{D^2}{2}\ln\pi+\frac D2\ln 2+\frac {ND}2\ln\frac N2-\sum_{j=1}^D{\left(\frac{j}2\left(\ln\frac N2+1\right)\right)}\nn\\
  %&\approx\frac{D^2}{2}\ln\pi+\frac D2\ln 2+\frac {ND}2\ln\frac N2-\frac{D(D+1)}4\left(\ln\frac N2+1\right)\nn\\
  %&\approx\frac{D^2}{2}\ln\pi-\frac {D(2N-3-D)}4\ln 2+\frac {D(2N-1-D)}4\ln N-\frac{D(D+1)}4\nn\\
...
\begin{align}
  ...
  %&\approx\frac N2\left(\ln|\Sigma|-D\ln\frac N2\right)+\frac {D(2N-1-D)}4\ln \frac N2\nn\\
  %&\qquad-\frac D2\ln\frac N{2\pi}+\frac{D^2}{2}\ln\pi+\frac {D}2\ln 2-\frac{D(D+1)}4\nn\\
  &\approx\frac N2\ln|\Sigma|-\frac {D(3+D)}4\ln \frac N2
  +\mathcal{O}(1)
  %&\qquad+\frac{D(D+1)}{2}\ln\pi+\frac {D}2\ln 2-\frac{D(D+1)}4\nn\\
  %Note that while I have computed the order-constant term commented above, it does not include the order-constant terms in \nu,\beta,etc,so it isn't meaningful as is.
\end{align}
